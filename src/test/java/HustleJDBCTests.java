...
public class HustleJDBCTests {
	...
	public void prepareStatementTest() {
		...
		try {
			...
			conn.prepareStatement("CREATE TABLE t (a INT, b INT);").executeUpdate();
			conn.prepareStatement("INSERT INTO t VALUES (1, 2);").executeUpdate();
			conn.prepareStatement("INSERT INTO t VALUES (3, 4);").executeUpdate();
			HustleResultSet resultSet = conn.prepareStatement("SELECT a, b FROM t;").executeQuery();
			...
			Assert.assertEquals(4, resultSet.getLong(2));
